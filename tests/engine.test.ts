...
describe('Scoring Engine — Bayesian posteriors & archetype priors', () => {
  ...
  it('maps to archetype and computes Bayesian credible intervals', () => {
  ...
  it('triggers veto and sets intrinsicRisk=0.97 on critical trust incident', () => {
...
describe('DAG Risk Propagation — bottom-up taint & blast radius', () => {
  it('propagates taint from a risky child up to its parent', () => {
    ...
    // Child: untouched effectiveRisk, blast radius = risk × dependents
    expect(child.effectiveRisk).toBe(0.97);
    expect(child.blastRadius).toBe(0.97 * 1);

    // Parent: inherited taint, risk floored at 0.8, score updated accordingly
    expect(parent.tainted).toBe(true);
    expect(parent.effectiveRisk).toBeGreaterThanOrEqual(0.8);
